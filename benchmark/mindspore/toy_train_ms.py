...
def get_model_and_dataloader(model_name, dataset_name, hidden_size):
    ...
    train_loader = GeneratorDataset(
        train_dataset, column_names="input_ids", shuffle=False
    ).batch(8)
...
if __name__ == "__main__":
    ...
    grad_fn = ms.value_and_grad(
        model, grad_position=None, weights=optimizer.parameters, has_aux=True
    )
    for epoch in range(epoch):
        for step, batch in enumerate(train_loader.create_tuple_iterator()):
            (input_ids,) = batch
            (loss, _), grads = grad_fn(
                input_ids=input_ids, labels=input_ids, return_dict=False
            )
            ms.synchronize()
            start = time.time()
            optimizer(grads)
            ...
            lr_scheduler.step()
            logger.info(
                f"Epoch: {epoch} Step: {step} LR: {optimizer.param_groups[0]['lr'].item():.5f} Optimizer update time: {duration:.3f} Training loss: {loss.item()}"
            )
